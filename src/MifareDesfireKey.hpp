...
class MifareDesfireKey {
  ...
  public:
    ...
    const static MifareDesfireKey create_3des_key_with_version(uint8_t value[16]) {
    ...
    const static MifareDesfireKey create_3k3des_key_with_version(uint8_t value[24]) {
      return MifareDesfireKey(mifare_desfire_3k3des_key_new_with_version(value));
    };
    ...
    static MifareDesfireKey create_aes_key_with_version(uint8_t value[16], uint8_t version) {
      return MifareDesfireKey(mifare_desfire_aes_key_new_with_version(value, version));
    };
};
